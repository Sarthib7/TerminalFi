...
export function useStablecoinPrices() {
  return useQuery<StablecoinPrice[]>({
    ...
    queryFn: async () => {
      ...
      // Try Jupiter as fallback (may require API key)
      ...
      try {
        ...
        // Jupiter API may require authentication - gracefully degrade
        console.warn("[Prices] ⚠️ Jupiter fetch failed (may require API key):", err);
      }
      ...
      return ASSET_LIST.map((asset) => {
        ...
        // Fallback chain: Pyth → Jupiter → 1.0 (default for stablecoins/RWAs)
        const price = pythData?.price ?? jupiterPrices[asset.mint] ?? (asset.category === "stablecoin" || asset.category === "rwa" ? 1.0 : 0);
        ...
        return {
          ...
          source: pythData ? ("pyth" as const) : jupiterPrices[asset.mint] ? ("jupiter" as const) : ("fallback" as const),
          updatedAt: Date.now(),
        };
      });
    },
